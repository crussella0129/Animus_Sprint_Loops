...
## 2026-07-03 — Bundle parity is guarded, not deduplicated; testing lives in GitHub via one canonical runner (sprint 11)
- **Context:** Sprint 11 (self-review/refactor) found: the four bundles' shared assets were byte-identical only by hand-discipline (no guard); the sprint-8 "top backlog item" (CI) had never shipped, making "merge green-CI PRs" vacuous; the persistent backlog was empty, so carry-forwards evaporated into decisions.md prose. Mid-plan, the user directed a review of their array-test design repo (Merkle DAG of content-addressed test confirmations) with "adopt what you like; work CI in."
- **Decision:**
  (a) **Parity guard over deduplication:** `tools/check-bundle-sync.sh` enforces the measured parity map (scripts+schemas ×4, prompts ×3, phases 00/01/02/04/05 claude↔codex; 03/06 + condensed rewrites intentionally divergent). The sprint-2 bundle-atomicity ADR stands — copies stay physical, the guard makes them safe.
  (b) **One canonical suite runner:** `tools/run-guards.sh` is the single definition of "the suite" for the local Test phase AND CI (`.github/workflows/ci.yml`, push + PR-to-main). Array-test-derived, sized to bash: per-suite ndjson confirmations with normalized-output evidence hashes, and a `--determinism` double-run meta-check. Deferred until array-test's engine (T1–T5) ships: content-addressed cell keys/memoization, committed Merkle ledger, property tiers (rationale in ROADMAP §1).
  (c) **CI conclusion is the authoritative confirmation:** test-report schema (×4) gains a `## CI Confirmation` block (head SHA, run URL, conclusion, confirmations reference, no-CI fallback); phase-05 directs Test phases at the canonical runner.
  (d) **Carry-forwards are backlog, not prose:** `(backlog)` entry form documented in the agent-tasks schema (×4) + loop docs; ROADMAP.md carries rationale; T-101..T-109 seeded (T-109: user-requested dev-branch working model + approve-vs-auto-merge election at launch).
- **Alternatives considered:** true deduplication via shared source or symlinks (rejected — violates bundle atomicity, breaks plugin auto-discovery); Merkle root/ledger committed per CI run (rejected for now — commit churn; GitHub's immutable run logs + artifacts give the audit surface at this scale); ROADMAP-only trajectory without backlog seeding (rejected — the observed failure mode was precisely prose that never became actionable).
- **Consequences:**
  - Any single-bundle edit to a mapped asset now fails CI (bundle-sync suite) — the parity model is enforced, not hoped.
  - The autonomy policy's "merge on green CI" is meaningful for the first time; this sprint's PR is the first gated by it.
