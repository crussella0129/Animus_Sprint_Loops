...
## T-165 (sprint 20)
- **Description:** Added `scaffold-ci.sh`, which renders detected languages into the host's own CI format. Four hosts, two formats: `github`, `gitea`, and `forgejo` share one Actions renderer differing only by directory, `gitlab` gets `.gitlab-ci.yml`, `generic` gets an executable `ci.sh`, and `local-only` gets nothing. No-clobber is directory-level for the Actions hosts — if the workflow directory holds anything at all, nothing is written — because a file-level check would drop a generated workflow beside a hand-written one and leave two CI systems disagreeing about the same push. Triggers are passed in by the caller and named for both the profile's branches; a workflow triggering only on base never runs on the branch sprints commit to, nor on the checkpoint. When a canonical runner is present it replaces the per-language jobs entirely. Generated commands are chosen to pass on an empty-but-valid project, since a job that is red on day one teaches an operator to ignore red: `npm test --if-present` is a no-op without a test script, and the Python step tolerates exactly pytest's exit 5 and nothing else. Nine fixtures, including `test_scaffold_generic_ci_actually_fails`, which **executes** the generated script — the only host where that is possible — and proves a clean project passes and a project with a broken shell script fails. Two fixture defects were found and fixed during the task: the executable proof originally used a `package.json` whose contents were the literal word `fixture`, so `npm install` failed for the wrong reason, and a leftover no-op branch broke shellcheck.
- **Intent:** [INT-0012](../intents/INT-0012-ci-scaffolding-lifecycle.md)
...
- **Commit:** `f21976560efcdbb8b820e50a37f7751160829296`

## T-175 (sprint 21)
- **Intent:** [INT-0013](../intents/INT-0013-verification-integrity.md)
- **Description:** Two version literals in deploy-substrate.test.sh became relationships against BOOK_SUBSTRATE_CONTRACT_VERSION: the ahead-Book seed is now V+1 rather than 99, and the preview seed V-1 rather than 3. The negative-assertion audit found the enumeration itself to be a weak instrument - it flagged 51 sites, and inspection of the flagged fixtures in deploy-substrate, remote-adapter, check-tracked and detect-languages found them already paired, largely by the sprint 19 and 20 repairs. Recorded as a count, not treated as a verdict. A different check did find real defects, by cross-referencing each suite's `pass` labels against the names its assertions fail under: remote-adapter's test_checkpoint_reopen_is_inert and remote-profile's test_profile_enum_diagnostic_names_every_value both had assertions that would fail under a DIFFERENT fixture's name, sending a reader to the wrong test, and book-routing's test_routing_unchanged_for_unstamped_book asserted but was never reported as passing. All three fixed. The mechanical gate is the sensitivity sweep, which on its first real run over the whole corpus reported exactly one INSENSITIVE suite: merge-policy-test. The suite was fine; the subject mapping T-174 gave it was wrong, naming a sprint-14 compatibility shim rather than the script the suite actually exercises. Corrected, after which it scores sensitive and the sweep is clean. The duplication that finding exposed - the runner executing check-adapter-semantics.sh and its fixtures twice on every run since sprint 14, at 805s per fixture run - is recorded as T-177 rather than fixed here.
- **Completed:** 2026-09-03T00:00:00Z
