...
SUITES=(selftest merge-policy merge-policy-test plugin-manifest plugin-manifest-test bundle-sync bundle-sync-test adapter-semantics adapter-semantics-test operator-docs remote-profile check-substrate check-tracked detect-languages scaffold-ci deploy-substrate remote-adapter sync-work-branch shellcheck)
if [ -n "$EXTRA_DIR" ]; then
