...
out=$(PATH="$STUB_BIN:$PATH" STUB_PR_EXISTS=1 bash "$RA" --root "$P2" open-pr 2>&1) ||
  die test_pr_refuses_second 'open-pr errored'
grep -q 'pr create' "$STUBLOG" && die test_pr_refuses_second 'opened a second PR'
printf '%s' "$out" | grep -qi 'already' || die test_pr_refuses_second 'no already-open message'
pass test_pr_refuses_second
