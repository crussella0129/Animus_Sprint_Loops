...
8. *(Optional — PR-wrapped sprints)* If this sprint was developed on a feature
   ...
   - **On CI green** (verified per `phases/05-test-phase.md`'s CI verify
     ...
     you cannot tell what merging actually sets in motion.
...
Finally, return to Initialize Sprint. Claude Code may use its recurring
invocation primitive described in `SKILL.md` to drive the next bounded
iteration.
