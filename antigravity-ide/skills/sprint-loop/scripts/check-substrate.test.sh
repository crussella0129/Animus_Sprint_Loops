#!/usr/bin/env bash
# Fixtures for check-substrate.sh — complete/absent/partial + read-only + edges.
set -uo pipefail
...
C="$TMP_ROOT/complete"; git_init_branches "$C" main dev bump; make_book "$C"
make_profile "$C" 'provider: github
base: main
work: dev
bump: bump'
[ "$(run "$C")" = substrate-complete ] || die test_substrate_complete "got '$(run "$C")'"
bash "$CS" --root "$C" >/dev/null 2>&1 || die test_substrate_complete 'exit non-zero for complete'
pass test_substrate_complete
...
R="$TMP_ROOT/ro"; git_init_branches "$R" main dev bump; make_book "$R"
...
# test_substrate_complete_without_bump
WB="$TMP_ROOT/nobump"; git_init_branches "$WB" main dev; make_book "$WB"
make_profile "$WB" 'provider: github
base: main
work: dev'
[ "$(run "$WB")" = substrate-complete ] || die test_substrate_complete_without_bump "got '$(run "$WB")'"
pass test_substrate_complete_without_bump
